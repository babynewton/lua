...
static int findindex (lua_State *L, Table *t, StkId key) {
  int i;
  if (key->is_nil()) return -1;  /* first iteration */
  i = arrayindex(key);
...
int luaH_next (lua_State *L, Table *t, StkId key) {
  ...
  for (i++; i < t->sizearray; i++) {  /* try first array part */
    if (!(&t->array[i])->is_nil()) {  /* a non-nil value? */
      ...
      (key+1)->set_obj(L, &t->array[i]);
      return 1;
    }
  }
  for (i -= t->sizearray; i < sizenode(t); i++) {  /* then hash part */
    if (!(gval(gnode(t, i)))->is_nil()) {  /* a non-nil value? */
      key->set_obj(L, gkey(gnode(t, i)));
      (key+1)->set_obj(L, gval(gnode(t, i)));
      return 1;
    }
  }
...
void luaH_resize (lua_State *L, Table *t, int nasize, int nhsize) {
  ...
  for (i = twoto(oldhsize) - 1; i >= 0; i--) {
    ...
    if (!(gval(old))->is_nil()) {
      ...
      (luaH_set(L, t, gkey(old)))->set_obj(L, gval(old));
    }
  }
  if (!isdummy(nold))
...
TValue *luaH_newkey (lua_State *L, Table *t, const TValue *key) {
  ...
  (gkey(mp))->set_obj(L, key);
  luaC_barrierback(L, obj2gco(t), key);
  lua_assert(gval(mp)->is_nil());
  return gval(mp);
}
...
static int unbound_search (Table *t, unsigned int j) {
  ...
  while (!((TValue*)luaH_getint(t, j))->is_nil()) {
    i = j;
    j *= 2;
    if (j > cast(unsigned int, MAX_INT)) {  /* overflow? */
      /* table was built with bad purposes: resort to linear search */
      i = 1;
      while (!((TValue*)luaH_getint(t, i))->is_nil()) i++;
      return i - 1;
    }
  }
  ...
  while (j - i > 1) {
    ...
    if (((TValue*)luaH_getint(t, m))->is_nil()) j = m;
    else i = m;
  }
  return i;
}
...
int luaH_getn (Table *t) {
  ...
  if (j > 0 && (&t->array[j - 1])->is_nil()) {
    /* there is a boundary in the array part: (binary) search for it */
    unsigned int i = 0;
    while (j - i > 1) {
      unsigned int m = (i+j)/2;
      if ((&t->array[m - 1])->is_nil()) j = m;
      else i = m;
    }
    return i;
  }
