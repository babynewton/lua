...
static const char *findvararg (CallInfo *ci, int n, StkId *pos) {
  int nparams = (ci->func)->to_l_closure()->p->numparams;
...
LUA_API const char *lua_getlocal (lua_State *L, const lua_Debug *ar, int n) {
  ...
  if (ar == NULL) {  /* information about non-active function? */
    if (!(L->top - 1)->is_l_closure())  /* not a Lua function? */
      ...
      name = luaF_getlocalname((L->top - 1)->to_l_closure()->p, n, 0);
  }
...
static void collectvalidlines (lua_State *L, Closure *f) {
  if (noLuaClosure(f)) {
    ...
    L->top->set_value(L, t);  /* push it on stack */
    api_incr_top(L);
...
LUA_API int lua_getinfo (lua_State *L, const char *what, lua_Debug *ar) {
  ...
  if (*what == '>') {
    ci = NULL;
    func = L->top - 1;
    api_check(L, func->is_function(), "function expected");
    what++;  /* skip the '>' */
    L->top--;  /* pop function */
  }
  else {
...
l_noret luaG_errormsg (lua_State *L) {
  if (L->errfunc != 0) {  /* is there an error handling function? */
    ...
    if (!errfunc->is_function()) luaD_throw(L, LUA_ERRERR);
    L->top->set_obj(L, L->top - 1);  /* move argument */
    (L->top - 1)->set_obj(L, errfunc);  /* push function */
    L->top++;
